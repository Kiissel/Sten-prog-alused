...
def insert_tuple_inside_tuple(outer_tuple: tuple, position: int, inner_tuple: tuple) -> tuple:
    ...
    """
    if not isinstance(outer_tuple, tuple) or not isinstance(inner_tuple, tuple):
        raise ValueError("Both outer_tuple and inner_tuple must be tuples")

    if not (0 <= position <= len(outer_tuple)):
        raise ValueError("Position should be non-negative and at most equal to the length of outer_tuple")

    # Split the outer_tuple into two parts at the specified position
    first_part = outer_tuple[:position]
    second_part = outer_tuple[position:]

    # Concatenate the first part, inner_tuple, and the second part
    result_tuple = first_part + inner_tuple + second_part
